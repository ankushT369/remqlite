...
CFLAGS.intree_includes = \
    ...
    -I$(TOP)/ext/misc -I$(TOP)/ext/mongoose
...
coretestprogs:	testfixture$(B.exe) remqlite$(B.exe)
...
#
tool-zip:	testfixture$(T.exe) remqlite$(T.exe) sqldiff$(T.exe) \
            sqlite3_analyzer$(T.exe) sqlite3_rsync$(T.exe) $(TOP)/tool/mktoolzip.tcl
	strip remqlite$(T.exe) sqldiff$(T.exe) sqlite3_analyzer$(T.exe) sqlite3_rsync$(T.exe)
	./testfixture$(T.exe) $(TOP)/tool/mktoolzip.tcl
snapshot-zip:	testfixture$(T.exe) remqlite$(T.exe) sqldiff$(T.exe) \
            sqlite3_analyzer$(T.exe) sqlite3_rsync$(T.exe) $(TOP)/tool/mktoolzip.tcl
	strip remqlite$(T.exe) sqldiff$(T.exe) sqlite3_analyzer$(T.exe) sqlite3_rsync$(T.exe)
...
#
remqlite$(T.exe):	shell.c sqlite3.c $(TOP)/ext/mongoose/mongoose.c
	$(T.link) -o $@ \
		shell.c sqlite3.c $(TOP)/ext/mongoose/mongoose.c \
...
#
remqlite$(T.exe)-1:
remqlite$(T.exe)-0: remqlite$(T.exe)
all: remqlite$(T.exe)-$(HAVE_WASI_SDK)
...
install-shell-0: remqlite$(T.exe) $(install-dir.bin)
	$(INSTALL) remqlite$(T.exe) "$(install-dir.bin)"
install-shell-1:
...
SHELL_DEP = \
    ...
    $(TOP)/ext/mongoose/mongoose.c \
    $(TOP)/ext/mongoose/mongoose.h
...
#
tidy:
	rm -f *.o *.obj *.c *.da *.bb *.bbg gmon.* *.rws remqlite$(T.exe)
